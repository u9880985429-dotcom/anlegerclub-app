/**
 * ablefy-Modul · SYNC-SERVICE — die komplette Domaenenlogik des Voll-Syncs.
 *
 * Vorher lebte all das (Aggregation, Einsammeln, Persistieren, Pagination,
 * Event-Logs) direkt im POST-Handler von `app/api/v1/ablefy/sync/route.ts`.
 * Diese Datei kapselt es 1:1 — Verhalten, Reihenfolge, Events und Resultate
 * sind identisch zur vorherigen Inline-Version. Der Route-Handler ruft nur noch
 * `runAblefySync(...)` auf und uebersetzt das Resultat in die passende Response.
 *
...
import { createAblefyApiClient } from "./api-client";
import { loadAblefyConfigFromDb } from "./config-repository";

/** Eingabe-Parameter fuer einen Sync-Lauf (entspricht dem geparsten Request-Body). */
export interface RunAblefySyncParams {
  apiKey: string;
  apiSecret: string;
...
/** Erfolgreiches Sync-Resultat — das KPI-Aggregat. */
export interface AblefySyncOk {
  ok: true;
  aggregate: AggregatedKpi;
}

/** Upstream-HTTP-Fehler waehrend der Pagination (vorher: 502 ablefy_http_error). */
export interface AblefySyncHttpError {
  ok: false;
  reason: "http";
  httpStatus: number;
  page: number;
...
/** Sonstiger Fehler (z.B. ungueltiges JSON / fetch-Fehler) (vorher: 502 mit msg). */
export interface AblefySyncFailure {
  ok: false;
  reason: "exception";
  message: string;
}

export type AblefySyncResult = AblefySyncOk | AblefySyncHttpError | AblefySyncFailure;
...
/**
 * Fuehrt den kompletten Ablefy-Voll-Sync aus:
 *   1. Mapping aus der DB laden (Slug-Resolution; leer wenn nicht konfiguriert).
 *   2. Invoices ueber /api/invoices paginiert holen.
 *   3. KPI-Aggregat berechnen (nur gemappte Produkte).
 *   4. Customer + Subscription einsammeln und gebuendelt persistieren.
 *
 * Schreibt dabei die sync.started/failed/completed-Events (identisch zur
 * vorherigen Inline-Logik) und liefert entweder das KPI-Aggregat oder einen
 * klaren Fehler zurueck. Der Aufrufer (Route) baut daraus die HTTP-Response.
 */
export async function runAblefySync(params: RunAblefySyncParams): Promise<AblefySyncResult> {
  const {
    apiKey,
    apiSecret,
    dateFrom,
